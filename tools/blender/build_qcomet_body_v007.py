"""Q Comet v007 body surfacing — closed panoramic greenhouse, fragment armature.

Does not overwrite v001–v006. Builds fresh (no v002 load required).
Original fictional Q Motors / Q Comet identity only. No .yft/.ytd/.ybn export.
"""
...
# Fastback greenhouse stations: y, half_width, belt_z, roof_z
GREENHOUSE = (
    (1.10, 0.58, 0.98, 1.12),
    (0.78, 0.72, 0.96, 1.34),
    (0.12, 0.76, 0.95, 1.40),
...
PIVOTS = {
    "door_dside_f": (-0.97, 0.90, 0.67),
    "door_pside_f": (0.97, 0.90, 0.67),
    "door_dside_r": (-0.96, -0.18, 0.67),
    "door_pside_r": (0.96, -0.18, 0.67),
    "bonnet": (0.0, 1.14, 0.98),
    "boot": (0.0, -1.38, 0.94),
    "steering": (-0.38, 0.46, 1.02),
    "seat_dside_f": (-0.38, 0.12, 0.48),
    "seat_pside_f": (0.38, 0.12, 0.48),
    "seat_dside_r": (-0.38, -0.78, 0.48),
    "seat_pside_r": (0.38, -0.78, 0.48),
}
...
def set_origin_world(obj, world_location):
    """Move object origin to a world-space pivot without shifting geometry visually."""
...
def clear_scene():
    ...
    print("Cleared scene (factory empty)")
...
def panel_grid(name, y0, y1, z0, z1, x_sign, thickness, mat, collection, y_steps=8, z_steps=6, extra=0.006):
    ...
    for yi in range(y_steps - 1):
        ...
        faces.append(
            (
                idx(0, yi, z_steps - 1),
                idx(0, yi + 1, z_steps - 1),
                idx(1, yi + 1, z_steps - 1),
                idx(1, yi, z_steps - 1),
            )
        )
    for zi in range(z_steps - 1):
        ...
        faces.append(
            (
                idx(0, y_steps - 1, zi),
                idx(1, y_steps - 1, zi),
                idx(1, y_steps - 1, zi + 1),
                idx(0, y_steps - 1, zi + 1),
            )
        )
    return new_mesh_object(name, vertices, faces, mat, collection, bevel=0.004)


def glass_panel(name, y0, y1, z0, z1, x_sign, mat, collection, thickness=0.008, y_steps=7, z_steps=5, inset=0.035):
    """Beltline→roof glass sheet following body shoulder, inset from blackout pillars."""
    vertices = []
    faces = []
    for yi in range(y_steps):
        y = lerp(y0, y1, yi / (y_steps - 1))
        ...
        for zi in range(z_steps):
            t = zi / (z_steps - 1)
            z = lerp(z0, z1, t)
            # pull inward toward roof half-width as z rises (raked greenhouse)
            outer = side_x(station, z, extra=-inset)
            roof_pull = lerp(1.0, 0.82, t)
            x = x_sign * outer * roof_pull
            vertices.append((x, y, z))
    for yi in range(y_steps):
        y = lerp(y0, y1, yi / (y_steps - 1))
        station = station_at(y)
        for zi in range(z_steps):
            t = zi / (z_steps - 1)
            z = lerp(z0, z1, t)
            outer = side_x(station, z, extra=-inset - thickness)
            roof_pull = lerp(1.0, 0.82, t)
            x = x_sign * outer * roof_pull
            vertices.append((x, y, z))
...
def solid_pillar(name, y0, y1, z0, z1, x_sign, mat, collection, width=0.055, rake=0.0):
    """Opaque A/B/C pillar — solid blackout volume, not a thin glass post."""
    vertices = []
    steps = 6
    for i in range(steps):
        ...
        z = lerp(z0, z1, t)
        station = station_at(y)
        x_outer = x_sign * (side_x(station, z, extra=0.002) - 0.008)
        x_inner = x_outer - x_sign * width
        y_off = rake * (t - 0.5) * 0.08
        vertices.append((x_outer, y + y_off, z))
        vertices.append((x_inner, y + y_off, z))
    # Extrude slight thickness in Y at ends for volume — build quads along strip
    faces = []
    for i in range(steps - 1):
        a = i * 2
        b = (i + 1) * 2
        faces.append((a, a + 1, b + 1, b))
    # Add depth by duplicating strip offset in local Y-ish
    depth = 0.028
    n = len(vertices)
    back = []
    for i in range(steps):
        ...
        x_inner = x_outer - x_sign * width
        y_off = rake * (t - 0.5) * 0.08
        # push slightly inward in Y toward cabin center of span
        mid_y = (y0 + y1) * 0.5
        push = depth if y >= mid_y else -depth
        # actually push toward vehicle centerline in lateral? Keep Y offset toward mid-span
        toward = math.copysign(depth, mid_y - y) if abs(mid_y - y) > 1e-6 else 0.0
        back.append((x_outer, y + y_off + toward * 0.35, z))
        back.append((x_inner, y + y_off + toward * 0.35, z))
...
def build_greenhouse(mats, body, glass):
    """Closed raked fastback DLO: solid pillars + roof frame + separable glass. No bronze drip rails."""
    objects = []

    # Solid A pillars (raked)
    objects.append(solid_pillar("pillar_a_dside", 0.88, 1.14, 0.98, 1.34, -1.0, mats["blackout"], body, width=0.062, rake=-1.0))
    objects.append(solid_pillar("pillar_a_pside", 0.88, 1.14, 0.98, 1.34, 1.0, mats["blackout"], body, width=0.062, rake=-1.0))
    # Solid B pillars
    objects.append(solid_pillar("pillar_b_dside", -0.22, -0.06, 0.96, 1.38, -1.0, mats["blackout"], body, width=0.048, rake=0.0))
    objects.append(solid_pillar("pillar_b_pside", -0.22, -0.06, 0.96, 1.38, 1.0, mats["blackout"], body, width=0.048, rake=0.0))
    # Solid C sail pillars
    objects.append(solid_pillar("pillar_c_dside", -1.38, -1.08, 0.98, 1.22, -1.0, mats["blackout"], body, width=0.070, rake=1.0))
    objects.append(solid_pillar("pillar_c_pside", -1.38, -1.08, 0.98, 1.22, 1.0, mats["blackout"], body, width=0.070, rake=1.0))

    # Roof rails (opaque blackout, not bronze drip)
    objects.append(solid_pillar("roof_rail_dside", -1.20, 0.95, 1.34, 1.40, -1.0, mats["blackout"], body, width=0.038, rake=0.0))
    objects.append(solid_pillar("roof_rail_pside", -1.20, 0.95, 1.34, 1.40, 1.0, mats["blackout"], body, width=0.038, rake=0.0))
    objects.append(box("roof_header", -0.62, 0.62, 0.92, 1.10, 1.30, 1.40, mats["blackout"], body, 0.006))
    objects.append(box("roof_frame_rear", -0.50, 0.50, -1.36, -1.18, 1.08, 1.22, mats["blackout"], body, 0.006))

    # Separable GTA-named glass
    objects.append(box("windscreen", -0.60, 0.60, 0.90, 1.12, 0.99, 1.33, mats["cabin_glass"], glass, 0.003))
    objects.append(box("windscreen_r", -0.50, 0.50, -1.36, -1.12, 0.99, 1.18, mats["cabin_glass"], glass, 0.003))
    objects.append(glass_panel("window_lf", -0.08, 0.86, 0.98, 1.36, -1.0, mats["cabin_glass"], glass))
    objects.append(glass_panel("window_rf", -0.08, 0.86, 0.98, 1.36, 1.0, mats["cabin_glass"], glass))
    objects.append(glass_panel("window_lr", -1.20, -0.20, 0.97, 1.34, -1.0, mats["cabin_glass"], glass))
    objects.append(glass_panel("window_rr", -1.20, -0.20, 0.97, 1.34, 1.0, mats["cabin_glass"], glass))
    objects.append(loft_simple("panoramic_roof_glass", GREENHOUSE, 24, 6.2, mats["roof_glass"], glass))
    return objects
...
def rear_light_bar(emit_mat, brake_mat, indicator_mat, lens_mat, reflector_mat, collection):
    objects = []
    ...
    # Amber indicator segments at corners
    objects.append(box("indicator_rear_dside", -0.86, -0.68, -2.34, -2.26, 0.74, 0.86, indicator_mat, collection, 0.003))
    objects.append(box("indicator_rear_pside", 0.68, 0.86, -2.34, -2.26, 0.74, 0.86, indicator_mat, collection, 0.003))
    # Rear reflectors
    objects.append(box("reflector_dside", -0.78, -0.58, -2.36, -2.30, 0.52, 0.62, reflector_mat, collection, 0.003))
    objects.append(box("reflector_pside", 0.58, 0.78, -2.36, -2.30, 0.52, 0.62, reflector_mat, collection, 0.003))
    return objects
...
def make_wheel(name, location, tire_mat, rim_mat, bronze_mat, caliper_mat, collection):
    """Turbine multi-spoke: bronze outer face, dark barrel, Q+ hub, tread tire, disc, caliper."""
    parts = []
    sign = 1.0 if location[0] > 0 else -1.0
    tire_r = ENVELOPE["tire_diameter"] * 0.5
    tire_w = ENVELOPE["tire_width"]
    ...
    # Subtle tread grooves via thin rings
    for groove_i, gr in enumerate((0.92, 0.86, 0.80)):
        bpy.ops.mesh.primitive_torus_add(
            location=location,
            rotation=(0.0, math.radians(90.0), 0.0),
            major_radius=tire_r * gr,
            minor_radius=0.006,
            major_segments=36,
            minor_segments=8,
        )
        groove = bpy.context.object
        groove.name = f"{name}_tread_{groove_i + 1}"
        groove.data.materials.append(tire_mat)
        link_object(groove, collection)
        parts.append(groove)
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        radius=0.26,
        depth=tire_w * 0.55,
        location=(location[0] - sign * 0.02, location[1], location[2]),
        rotation=(0.0, math.radians(90.0), 0.0),
    )
    barrel = bpy.context.object
    barrel.name = name.replace("wheel_", "rim_barrel_")
    barrel.data.materials.append(rim_mat)
    link_object(barrel, collection)
    parts.append(barrel)

    # Bronze outer face ring
    hub_x = location[0] + sign * 0.055
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=36,
        radius=0.275,
        depth=0.018,
    ...
    face.name = name.replace("wheel_", "rim_face_")
    face.data.materials.append(bronze_mat)
    link_object(face, collection)
    parts.append(face)

    # Turbine multi-spoke (curved lean via rotation offset)
    for spoke_index in range(12):
        ...
        spoke.scale = (0.010, 0.195, 0.022)
        bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
        spoke.rotation_euler = (angle + 0.22, math.radians(90.0), math.radians(8.0 * sign))
        spoke.location = (
            hub_x,
            location[1] + math.cos(angle) * 0.105,
            location[2] + math.sin(angle) * 0.105,
        )
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        radius=0.055,
        depth=0.042,
        location=(hub_x + sign * 0.01, location[1], location[2]),
        rotation=(0.0, math.radians(90.0), 0.0),
    )
    ...
    parts.append(
        q_plus_badge(
            name.replace("wheel_", "hub_badge_"),
            (hub_x + sign * 0.032, location[1], location[2]),
            sign,
            bronze_mat,
            collection,
            scale=0.028,
    ...
    bpy.ops.mesh.primitive_cylinder_add(
        ...
        radius=0.18,
        depth=0.014,
        location=(location[0] - sign * 0.05, location[1], location[2]),
        rotation=(0.0, math.radians(90.0), 0.0),
    )
    disc = bpy.context.object
    disc.name = name.replace("wheel_", "brake_disc_")
    disc.data.materials.append(rim_mat)
    link_object(disc, collection)
    parts.append(disc)
...
def camera_mirror(side, carbon, bronze, emit_mat, collection):
    ...
    return body, stalk, accent, indicator


def seat(name, x, y, tan, olive, bronze, collection):
    ...
    badge = q_plus_badge(f"{name}_qplus", (x, y + 0.225, 1.08), 1.0, bronze, collection, scale=0.022)
    return cushion, back, bolster_l, bolster_r, head, badge


def steering_wheel(bronze, olive, collection):
    """Two-spoke flat-bottom wheel named steering; alias empty steeringwheel."""
    # Flat-bottom torus approximation: torus + cut via scale squash at bottom using mesh
    bpy.ops.mesh.primitive_torus_add(
    ...
    # Flatten bottom verts
    mesh = wheel.data
    for vert in mesh.vertices:
        # local Z after rotation is messy; nudge lowest world verts
        co = wheel.matrix_world @ vert.co
        if co.z < 0.92:
            vert.co = wheel.matrix_world.inverted() @ Vector((co.x, co.y, lerp(co.z, 0.92, 0.55)))
    mesh.update()
    hub = box("steering_hub", -0.43, -0.33, 0.44, 0.50, 0.97, 1.07, bronze, collection, 0.008)
    spoke_l = box("steering_spoke_l", -0.52, -0.28, 0.45, 0.49, 1.00, 1.04, bronze, collection, 0.004)
    spoke_r = box("steering_spoke_r", -0.48, -0.28, 0.45, 0.49, 0.96, 1.00, bronze, collection, 0.004)
...
def parent_to_bone(obj, armature, bone_name):
    ...
    # Keep current world transform after parenting
    obj.matrix_parent_inverse = (armature.matrix_world @ armature.pose.bones[bone_name].matrix).inverted()


def build_armature(collections_map):
    """Fragment armature named qcomet with exact bone list; parent moving parts."""
    arm_data = bpy.data.armatures.new("qcomet_armature")
    arm_obj = bpy.data.objects.new("qcomet", arm_data)
    frag = make_collection("QCOMET_FRAGMENT")
    frag.objects.link(arm_obj)
    ...
    def add_bone(name, head, tip, parent=None):
        ...
        bone.tail = Vector(tip)
        if parent:
            bone.parent = edit_bones[parent]
        return bone

    add_bone("chassis", (0.0, 0.0, 0.20), (0.0, 0.0, 0.55))
    for door_name, pivot in (
        ("door_dside_f", PIVOTS["door_dside_f"]),
        ("door_pside_f", PIVOTS["door_pside_f"]),
        ("door_dside_r", PIVOTS["door_dside_r"]),
        ("door_pside_r", PIVOTS["door_pside_r"]),
    ):
        hx, hy, hz = pivot
        add_bone(door_name, (hx, hy, hz - 0.15), (hx, hy, hz + 0.15), "chassis")
    add_bone("bonnet", (*PIVOTS["bonnet"][:2], PIVOTS["bonnet"][2] - 0.02), (0.0, PIVOTS["bonnet"][1] - 0.25, PIVOTS["bonnet"][2]), "chassis")
    add_bone("boot", (*PIVOTS["boot"][:2], PIVOTS["boot"][2] - 0.02), (0.0, PIVOTS["boot"][1] + 0.25, PIVOTS["boot"][2]), "chassis")
    for wname, loc in WHEEL_LOCATIONS.items():
        add_bone(wname, (loc[0], loc[1], loc[2]), (loc[0] + (0.12 if loc[0] > 0 else -0.12), loc[1], loc[2]), "chassis")
    sx, sy, sz = PIVOTS["steering"]
    add_bone("steering", (sx, sy, sz), (sx, sy + 0.12, sz + 0.04), "chassis")
    for sname in ("seat_dside_f", "seat_pside_f", "seat_dside_r", "seat_pside_r"):
        px, py, pz = PIVOTS[sname]
        add_bone(sname, (px, py, pz), (px, py, pz + 0.30), "chassis")

    bpy.ops.object.mode_set(mode="OBJECT")

    # Parent primary meshes to bones
    bindings = {
        "chassis": ["chassis", "bodyshell"],
        "door_dside_f": ["door_dside_f", "window_lf", "handle_dside_f", "door_card_dside_f", "mirror_dside", "mirror_dside_stalk", "mirror_dside_bronze", "mirror_dside_indicator"],
        "door_pside_f": ["door_pside_f", "window_rf", "handle_pside_f", "door_card_pside_f", "mirror_pside", "mirror_pside_stalk", "mirror_pside_bronze", "mirror_pside_indicator"],
        "door_dside_r": ["door_dside_r", "window_lr", "handle_dside_r", "door_card_dside_r"],
        "door_pside_r": ["door_pside_r", "window_rr", "handle_pside_r", "door_card_pside_r"],
        "bonnet": ["bonnet"],
        "boot": ["boot"],
        "steering": ["steering", "steering_hub", "steering_spoke_l", "steering_spoke_r"],
        "seat_dside_f": ["seat_dside_f", "seat_dside_f_cushion", "seat_dside_f_back", "seat_dside_f_bolster_l", "seat_dside_f_bolster_r", "seat_dside_f_qplus"],
        "seat_pside_f": ["seat_pside_f", "seat_pside_f_cushion", "seat_pside_f_back", "seat_pside_f_bolster_l", "seat_pside_f_bolster_r", "seat_pside_f_qplus"],
        "seat_dside_r": ["seat_dside_r", "seat_dside_r_cushion", "seat_dside_r_back", "seat_dside_r_bolster_l", "seat_dside_r_bolster_r", "seat_dside_r_qplus"],
        "seat_pside_r": ["seat_pside_r", "seat_pside_r_cushion", "seat_pside_r_back", "seat_pside_r_bolster_l", "seat_pside_r_bolster_r", "seat_pside_r_qplus"],
    }
    for wname in WHEEL_LOCATIONS:
        bindings[wname] = [wname]

    for bone_name, obj_names in bindings.items():
        for obj_name in obj_names:
            obj = bpy.data.objects.get(obj_name)
            if obj is None:
                continue
            try:
                parent_to_bone(obj, arm_obj, bone_name)
            except Exception as exc:
                print(f"WARN parent {obj_name} -> {bone_name}: {exc}")

    # Verify bone names
    bone_names = [b.name for b in arm_data.bones]
    missing = [n for n in BONE_NAMES if n not in bone_names]
    if missing:
        raise RuntimeError(f"Missing bones: {missing}")
    print(f"Armature qcomet bones OK: {len(bone_names)}")
    return arm_obj


def build_collision_and_lod(mats):
    col = make_collection("QCOMET_COLLISION")
    for name, dims in (
        ("col_chassis", (-0.90, 0.90, -2.20, 2.20, 0.12, 0.95)),
        ("col_door_dside_f", (-1.00, -0.85, -0.16, 0.90, 0.36, 1.00)),
        ("col_door_pside_f", (0.85, 1.00, -0.16, 0.90, 0.36, 1.00)),
        ("col_door_dside_r", (-1.00, -0.85, -1.26, -0.18, 0.36, 0.98)),
        ("col_door_pside_r", (0.85, 1.00, -1.26, -0.18, 0.36, 0.98)),
        ("col_bonnet", (-0.80, 0.80, 1.14, 2.26, 0.70, 1.00)),
        ("col_boot", (-0.75, 0.75, -2.24, -1.38, 0.70, 0.98)),
        ("col_windscreen", (-0.62, 0.62, 0.90, 1.12, 0.98, 1.34)),
        ("col_glass_side", (-0.78, 0.78, -1.22, 0.86, 0.98, 1.36)),
    ):
        obj = box(name, *dims, mats["blackout"], col, bevel=0.0)
        obj.display_type = "WIRE"
        obj.hide_render = True

    for lod_name, size in (
        ("QCOMET_LOD_VH", 0.20),
        ("QCOMET_LOD_H", 0.16),
        ("QCOMET_LOD_M", 0.12),
        ("QCOMET_LOD_L", 0.08),
    ):
        lod_col = make_collection(lod_name)
        root = empty(f"{lod_name}_root", (0.0, 0.0, 0.70), lod_col, size=size)
        proxy = box(
            f"{lod_name}_proxy",
            -ENVELOPE["width"] * 0.45,
            ENVELOPE["width"] * 0.45,
            -ENVELOPE["length"] * 0.45,
            ENVELOPE["length"] * 0.45,
            0.15,
            ENVELOPE["height"] * 0.9,
            mats["olive"],
...
def build_body(mats):
    ...
    # Alias empty for bodyshell naming convention
    bodyshell = empty("bodyshell", (0.0, 0.0, 0.55), body, size=0.18)
    bodyshell.parent = chassis

    build_greenhouse(mats, body, glass)

    door_df = panel_grid("door_dside_f", -0.16, 0.90, 0.36, 0.98, -1.0, 0.013, mats["olive"], body)
    door_pf = panel_grid("door_pside_f", -0.16, 0.90, 0.36, 0.98, 1.0, 0.013, mats["olive"], body)
    door_dr = panel_grid("door_dside_r", -1.26, -0.18, 0.37, 0.96, -1.0, 0.013, mats["olive"], body)
    door_pr = panel_grid("door_pside_r", -1.26, -0.18, 0.37, 0.96, 1.0, 0.013, mats["olive"], body)
    ...
    bonnet = hood_panel(mats["olive"], body)
    boot = boot_panel(mats["olive"], body)

    # Set hinge origins
    set_origin_world(door_df, PIVOTS["door_dside_f"])
    set_origin_world(door_pf, PIVOTS["door_pside_f"])
    set_origin_world(door_dr, PIVOTS["door_dside_r"])
    set_origin_world(door_pr, PIVOTS["door_pside_r"])
    set_origin_world(bonnet, PIVOTS["bonnet"])
    set_origin_world(boot, PIVOTS["boot"])

    front_bumper = box("front_bumper", -0.90, 0.90, 2.16, 2.43, 0.20, 0.54, mats["olive"], body, 0.02)
    empty("bumper_f", front_bumper.location, body, size=0.1).parent = front_bumper
    dimple_intake(mats["olive"], body)
    box("front_lower_intake", -0.55, 0.55, 2.28, 2.42, 0.22, 0.32, mats["carbon"], body, 0.008)
    ...
    rear_light_bar(mats["tail"], mats["brake"], mats["indicator"], mats["lens"], mats["reflector"], lights)

    for name, location in WHEEL_LOCATIONS.items():
        make_wheel(name, location, mats["tire"], mats["rim"], mats["bronze"], mats["caliper"], running)
    ...
    box("dash_cluster", -0.55, -0.22, 0.50, 0.60, 0.94, 1.01, mats["screen"], interior, 0.002)
    box("dash_center_icons", -0.14, 0.14, 0.50, 0.60, 0.94, 1.01, mats["screen"], interior, 0.002)
    box("dash_passenger_qplus", 0.28, 0.58, 0.50, 0.60, 0.94, 1.01, mats["screen"], interior, 0.002)
    q_plus_badge("dash_qplus_badge", (0.43, 0.605, 0.975), 1.0, mats["bronze"], interior, scale=0.018)
    ...
    seat("seat_dside_f", -0.38, 0.12, mats["tan"], mats["olive_int"], mats["bronze"], interior)
    seat("seat_pside_f", 0.38, 0.12, mats["tan"], mats["olive_int"], mats["bronze"], interior)
    seat("seat_dside_r", -0.38, -0.78, mats["tan"], mats["olive_int"], mats["bronze"], interior)
    seat("seat_pside_r", 0.38, -0.78, mats["tan"], mats["olive_int"], mats["bronze"], interior)
    steering, *_ = steering_wheel(mats["bronze"], mats["olive_int"], interior)
    set_origin_world(steering, PIVOTS["steering"])
    empty("steeringwheel", PIVOTS["steering"], interior, size=0.08).parent = steering

    for pname, ploc in PIVOTS.items():
        if pname.startswith("door_") or pname in ("bonnet", "boot"):
            empty(f"PIVOT_{pname}", ploc, dimensions)
...
def make_materials():
    return {
        ...
        "roof_glass": principled_material("M_QCOMET_GLASS_ROOF", (0.01, 0.012, 0.014), roughness=0.06, transmission=0.12, alpha=0.82),
        ...
        "headlight": principled_material(
            "M_QCOMET_HEADLIGHT",
            (0.9, 0.92, 0.85),
            roughness=0.18,
            emission=((0.95, 0.96, 0.88), 12.0),
        ),
        "tail": principled_material(
            "M_QCOMET_TAIL",
            (0.55, 0.04, 0.04),
            roughness=0.22,
            emission=((0.85, 0.05, 0.04), 8.0),
        ),
        "brake": principled_material(
            "M_QCOMET_BRAKE",
            (0.7, 0.05, 0.05),
            roughness=0.2,
            emission=((1.0, 0.08, 0.06), 16.0),
        ),
        "reverse": principled_material(
            "M_QCOMET_REVERSE",
            (0.85, 0.85, 0.82),
            roughness=0.2,
            emission=((0.9, 0.9, 0.88), 6.0),
        ),
        "indicator": principled_material(
            "M_QCOMET_INDICATOR",
            (0.85, 0.45, 0.08),
            roughness=0.22,
            emission=((1.0, 0.5, 0.08), 6.0),
        ),
        "reflector": principled_material(
            "M_QCOMET_REFLECTOR",
            (0.55, 0.08, 0.06),
            metallic=0.35,
            roughness=0.28,
            emission=((0.4, 0.05, 0.04), 1.5),
        ),
        ...
        "ambient": principled_material(
            "M_QCOMET_AMBIENT",
            (0.7, 0.45, 0.2),
            roughness=0.3,
            emission=((0.85, 0.5, 0.2), 3.0),
        ),
    }
...
def configure_scene(scene):
    ...
    scene["qcomet_status"] = "BODY_SURFACING_V007"
    scene["qcomet_dimensions_note"] = "Provisional envelope; awaiting owner dimension sheet"
    scene["qcomet_front_axis"] = "+Y"
    scene["qcomet_passenger_axis"] = "+X"
    scene["qcomet_build"] = "qcomet_body_v007"
...
def render_review_views(scene, review_dir):
    ...
    for filename, label, location, camera_type, ortho_scale in views:
        ...
        scene.render.stamp_note_text = f"Q COMET BODY V007 / {label}"
        path = os.path.join(review_dir, f"qcomet_body_v007_{filename}.png")
    ...
    sheet = bpy.data.images.new("QCOMET_V007_CONTACT_SHEET", width=width * 2, height=height * 2, alpha=True)
    sheet.pixels.foreach_set(canvas.ravel())
    sheet.file_format = "PNG"
    sheet.filepath_raw = os.path.join(review_dir, "qcomet_body_v007_contact_sheet.png")
    sheet.save()
...
def collect_stats(output_path, review_dir, render_paths):
    ...
    for col in bpy.data.collections:
        collections[col.name] = len(col.objects)

    arm = bpy.data.objects.get("qcomet")
    bone_names = []
    ...
    stats = {
        "version": "v007",
        "status": "BODY_SURFACING_V007",
        "object_count": len(bpy.data.objects),
        "mesh_object_count": len(mesh_objects),
        "mesh_object_names": sorted(mesh_names),
        "totals": {
            "vertices": total_verts,
            "faces": total_faces,
            "triangles": total_tris,
        },
        "per_collection_counts": collections,
        "bone_names": bone_names,
        "material_names": materials,
        "file_paths": {
            "blend": output_path,
            "review_dir": review_dir,
            "contact_sheet": os.path.join(review_dir, "qcomet_body_v007_contact_sheet.png"),
            "renders": render_paths or [],
            "stats": os.path.join(review_dir, "qcomet_body_v007_stats.json"),
        },
        "envelope": ENVELOPE,
    ...
    stats_path = os.path.join(review_dir, "qcomet_body_v007_stats.json")
...
def main():
    ...
    # Safety: never overwrite prior versions
    for prior in ("v001", "v002", "v003", "v004", "v005", "v006"):
        if prior in os.path.basename(output_path).lower() and "v007" not in os.path.basename(output_path).lower():
            raise RuntimeError(f"Refusing to overwrite prior version path: {output_path}")
    ...
    print(f"Saved Q Comet body v007: {output_path}")
    render_paths = render_review_views(scene, review_dir)
    collect_stats(output_path, review_dir, render_paths)
    bpy.ops.wm.save_mainfile()
    print("QCOMET_BODY_V007_OK")
