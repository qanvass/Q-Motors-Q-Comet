"""Q Comet v005 closed panoramic greenhouse from the v002 blockout.

Does not overwrite v001–v004. Rebuilds roof and pillars as one blacked-out
fastback DLO: raked A, thin B, C-sail, no bronze drip lid.
"""
...
def build_body(mats):
    ...
    loft_simple("panoramic_glass_greenhouse", GREENHOUSE, 24, 6.2, mats["roof_glass"], glass)
    box("glass_windshield", -0.62, 0.62, 0.82, 1.14, 0.98, 1.34, mats["cabin_glass"], glass, 0.004)
    box("glass_rear", -0.52, 0.52, -1.40, -1.08, 0.98, 1.18, mats["cabin_glass"], glass, 0.004)
    box("glass_side_dside", -0.76, -0.69, -1.22, 0.86, 0.98, 1.36, mats["cabin_glass"], glass, 0.002)
    box("glass_side_pside", 0.69, 0.76, -1.22, 0.86, 0.98, 1.36, mats["cabin_glass"], glass, 0.002)
    bpy.ops.mesh.primitive_cube_add(location=(-0.73, 0.96, 1.16))
    pillar_a_l = bpy.context.object
    pillar_a_l.name = "pillar_a_dside"
    pillar_a_l.scale = (0.012, 0.16, 0.19)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
    pillar_a_l.rotation_euler = (math.radians(-32.0), 0.0, 0.0)
    pillar_a_l.data.materials.append(mats["blackout"])
    link_object(pillar_a_l, body)
    bpy.ops.mesh.primitive_cube_add(location=(0.73, 0.96, 1.16))
    pillar_a_r = bpy.context.object
    pillar_a_r.name = "pillar_a_pside"
    pillar_a_r.scale = (0.012, 0.16, 0.19)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
    pillar_a_r.rotation_euler = (math.radians(-32.0), 0.0, 0.0)
    pillar_a_r.data.materials.append(mats["blackout"])
    link_object(pillar_a_r, body)
    box("pillar_b_dside", -0.75, -0.715, -0.16, -0.10, 0.98, 1.36, mats["blackout"], body, 0.001)
    box("pillar_b_pside", 0.715, 0.75, -0.16, -0.10, 0.98, 1.36, mats["blackout"], body, 0.001)
    bpy.ops.mesh.primitive_cube_add(location=(-0.72, -1.16, 1.12))
    pillar_c_l = bpy.context.object
    pillar_c_l.name = "pillar_c_dside"
    pillar_c_l.scale = (0.012, 0.18, 0.16)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
    pillar_c_l.rotation_euler = (math.radians(38.0), 0.0, 0.0)
    pillar_c_l.data.materials.append(mats["blackout"])
    link_object(pillar_c_l, body)
    bpy.ops.mesh.primitive_cube_add(location=(0.72, -1.16, 1.12))
    pillar_c_r = bpy.context.object
    pillar_c_r.name = "pillar_c_pside"
    pillar_c_r.scale = (0.012, 0.18, 0.16)
    bpy.ops.object.transform_apply(location=False, rotation=False, scale=True)
    pillar_c_r.rotation_euler = (math.radians(38.0), 0.0, 0.0)
    pillar_c_r.data.materials.append(mats["blackout"])
    link_object(pillar_c_r, body)
...
def configure_scene(scene):
    ...
    scene["qcomet_status"] = "BODY_SURFACING_V005"
...
def render_review_views(scene, review_dir):
    ...
    for filename, label, location, camera_type, ortho_scale in views:
        ...
        scene.render.stamp_note_text = f"Q COMET BODY V005 / {label}"
        path = os.path.join(review_dir, f"qcomet_body_v005_{filename}.png")
    ...
    sheet = bpy.data.images.new("QCOMET_V005_CONTACT_SHEET", width=width * 2, height=height * 2, alpha=True)
    sheet.pixels.foreach_set(canvas.ravel())
    sheet.file_format = "PNG"
    sheet.filepath_raw = os.path.join(review_dir, "qcomet_body_v005_contact_sheet.png")
    sheet.save()
...
def main():
    ...
    print(f"Saved Q Comet body v005: {output_path}")
    render_review_views(scene, review_dir)
    bpy.ops.wm.save_mainfile()
    print("QCOMET_BODY_V005_OK")
