"""First detailed Q Comet body pass from the v002 proportion blockout.

Loads qcomet_blockout_v002.blend, keeps that file untouched, archives the
blockout as wireframe reference, and writes qcomet_body_v003.blend.

Original Q Motors surfacing only. No real-vehicle or other-game geometry.
No .yft / .ytd / .ybn fabrication.
"""
...
STATIONS = (
    ...
    {"y": 1.16, "hw": 0.97, "hw_rk": 0.91, "hw_rf": 0.50, "z0": 0.14, "z_rk": 0.24, "z_belt": 0.94, "z_sh": 1.01, "z_rf": 1.14},
    {"y": 0.72, "hw": 0.97, "hw_rk": 0.91, "hw_rf": 0.66, "z0": 0.14, "z_rk": 0.23, "z_belt": 0.96, "z_sh": 1.03, "z_rf": 1.36},
    {"y": 0.08, "hw": 0.97, "hw_rk": 0.91, "hw_rf": 0.70, "z0": 0.14, "z_rk": 0.23, "z_belt": 0.96, "z_sh": 1.03, "z_rf": 1.43},
    {"y": -0.52, "hw": 0.96, "hw_rk": 0.90, "hw_rf": 0.68, "z0": 0.14, "z_rk": 0.23, "z_belt": 0.95, "z_sh": 1.02, "z_rf": 1.40},
    {"y": -1.02, "hw": 0.95, "hw_rk": 0.89, "hw_rf": 0.54, "z0": 0.14, "z_rk": 0.24, "z_belt": 0.92, "z_sh": 1.00, "z_rf": 1.20},
...
def headlight_blades(side, emit_mat, lens_mat, collection):
    ...
    blades = []
    for index, z in enumerate((0.60, 0.67, 0.74)):
        points = []
        for step in range(12):
            t = step / 11.0
            x = sign * lerp(0.46, 0.84, t)
            y = lerp(2.34, 2.18, t)
            wave = 0.011 * math.sin(t * math.pi * 2.15 + index * 0.7)
            points.append((x, y, z + wave))
        blades.append(ribbon(f"{prefix}_blade_{index + 1}", points, 0.018, 0.007, emit_mat, collection))
    housing = box(
        f"{prefix}_housing",
        sign * 0.44 if sign > 0 else sign * 0.86,
        sign * 0.86 if sign > 0 else sign * 0.44,
        2.16,
        2.36,
        0.56,
        0.79,
...
def camera_mirror(side, carbon, bronze, emit_mat, collection):
    ...
    y = 0.92
    z = 1.04
    body = box(f"mirror_{side}", x - sign * 0.02, x + sign * 0.08, y - 0.05, y + 0.05, z - 0.018, z + 0.018, carbon, collection, 0.006)
    accent = box(f"mirror_{side}_bronze", x - sign * 0.01, x + sign * 0.07, y - 0.04, y + 0.04, z - 0.022, z - 0.016, bronze, collection, 0.002)
    indicator = box(f"mirror_{side}_indicator", x - sign * 0.005, x + sign * 0.06, y + 0.04, y + 0.055, z - 0.006, z + 0.006, emit_mat, collection, 0.001)
...
def build_body(mats):
    ...
    loft_simple("panoramic_glass_greenhouse", GREENHOUSE, 20, 2.8, mats["roof_glass"], glass)
    box("glass_windshield", -0.62, 0.62, 0.78, 1.16, 0.96, 1.28, mats["cabin_glass"], glass, 0.008)
    box("glass_rear", -0.58, 0.58, -1.42, -1.08, 0.94, 1.22, mats["cabin_glass"], glass, 0.008)
    box("glass_side_dside", -0.86, -0.78, -1.12, 0.86, 0.96, 1.28, mats["cabin_glass"], glass, 0.004)
    box("glass_side_pside", 0.78, 0.86, -1.12, 0.86, 0.96, 1.28, mats["cabin_glass"], glass, 0.004)
    box("pillar_a_dside", -0.80, -0.72, 0.86, 1.08, 0.94, 1.30, mats["blackout"], body, 0.006)
    box("pillar_a_pside", 0.72, 0.80, 0.86, 1.08, 0.94, 1.30, mats["blackout"], body, 0.006)
    box("pillar_c_dside", -0.78, -0.70, -1.28, -1.02, 0.92, 1.24, mats["blackout"], body, 0.006)
    box("pillar_c_pside", 0.70, 0.78, -1.28, -1.02, 0.92, 1.24, mats["blackout"], body, 0.006)
...
def make_materials():
    return {
        "olive": principled_material("M_QCOMET_OLIVE_BODY", (0.22, 0.25, 0.14), metallic=0.28, roughness=0.42, coat=0.08),
        ...
        "cabin_glass": principled_material("M_QCOMET_GLASS_CABIN", (0.04, 0.07, 0.08), roughness=0.04, transmission=0.85, alpha=0.28),
        "roof_glass": principled_material("M_QCOMET_GLASS_ROOF", (0.02, 0.03, 0.035), roughness=0.05, transmission=0.7, alpha=0.42),
...
def configure_scene(scene):
    ...
    scene["qcomet_status"] = "BODY_SURFACING_V003"
...
def render_review_views(scene, review_dir):
    ...
    for filename, label, location, camera_type, ortho_scale in views:
        ...
        scene.render.stamp_note_text = f"Q COMET BODY V003 / {label}"
        path = os.path.join(review_dir, f"qcomet_body_v003_{filename}.png")
    ...
    sheet = bpy.data.images.new("QCOMET_V003_CONTACT_SHEET", width=width * 2, height=height * 2, alpha=True)
    sheet.pixels.foreach_set(canvas.ravel())
    sheet.file_format = "PNG"
    sheet.filepath_raw = os.path.join(review_dir, "qcomet_body_v003_contact_sheet.png")
    sheet.save()
...
def main():
    ...
    print(f"Saved Q Comet body v003: {output_path}")
    render_review_views(scene, review_dir)
    bpy.ops.wm.save_mainfile()
    print("QCOMET_BODY_V003_OK")
